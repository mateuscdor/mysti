...
handler.before = async function (m) {
    ...
    if (!(id in this.math)) return conn.sendButton(m.chat, 'el asunto ha terminado', author, null, [['math', '/math']], m)
    if (m.quoted.id == this.math[id][0].id) {
        ...
        if (m.text == math.result) {
            ...
            if (--this.math[id][2] == 0) {
                clearTimeout(this.math[id][3])
                delete this.math[id]
                conn.sendButton(m.chat, `*¡Se acabo la oportunidad!*\nResponder: *${math.result}*`, author, null, [['again', `/math ${math.mode}`]], m)
